...
const HealthSafety = () => {
  ...
  const BenefitItem = ({ children, index }) => (
    <li className="flex items-start text-amber-800 mb-4">
      <div className="w-8 h-8 bg-amber-100 rounded-full flex items-center justify-center mr-3 flex-shrink-0 mt-1">
  ...
  const SectionHeader = ({ icon, title }) => {
    ...
    return (
      <div className="flex items-center mb-8 group">
        <div className="w-12 h-12 bg-amber-100 rounded-full flex items-center justify-center mr-5">
          <Icon className="w-6 h-6 text-amber-600" />
        </div>
        <h2 className="text-2xl md:text-3xl font-bold text-amber-900 relative">
          {title}
          <span className="absolute -bottom-1 left-0 w-0 h-1 bg-amber-400 transition-all duration-300 group-hover:w-full"></span>
        </h2>
      </div>
    );
  };
  ...
  return (
    <div className="bg-gradient-to-br from-amber-50 to-amber-100 font-sans text-amber-800">
      ...
      <div className="relative h-96 w-full">
        ...
        <div className="absolute inset-0 bg-amber-900 bg-opacity-30 flex items-center justify-center">
          <div className="text-center px-6">
            <h1 className="text-4xl md:text-5xl font-extrabold text-white mb-4 drop-shadow-lg relative">
              Ensuring Quality and Safety in Healthcare
              <span className="absolute -bottom-1 left-0 w-full h-2 bg-amber-400"></span>
            </h1>
            <p className="text-xl text-amber-100 max-w-3xl mx-auto drop-shadow-md">
      ...
      <div className="max-w-6xl mx-auto px-6 lg:px-8 py-16 -mt-20 relative z-10">
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-amber-200">
          ...
          {/* Intro Section */}
          <section className="px-8 py-12 sm:px-12 text-center hover:bg-amber-50/30 transition-colors duration-300">
          ...
          <section className="px-8 py-12 sm:px-12 bg-amber-100/50 hover:bg-amber-100/70 transition-colors duration-300">
            ...
            <div className="space-y-6">
              {[
                ...
                <div 
                  key={index}
                  className="p-6 bg-white rounded-xl border border-amber-200 shadow-sm hover:shadow-md hover:border-amber-300 transition-all duration-300"
                >
                  <strong className="block text-xl font-semibold text-amber-900 mb-2">{item.title}</strong>
                  <p className="text-amber-800">{item.description}</p>
                </div>
              ))}
            </div>
            <p className="mt-8 text-lg text-amber-800">
          ...
          <section className="px-8 py-12 sm:px-12 hover:bg-amber-50/30 transition-colors duration-300">
            <SectionHeader icon={BarChart} title="Key Benefits of ISO Standards" />
            <div className="grid md:grid-cols-2 gap-8">
              <div className="bg-amber-100/50 p-8 rounded-xl border border-amber-200 shadow-sm hover:shadow-md hover:border-amber-300 transition-all duration-300">
          ...
          <section className="px-8 py-12 sm:px-12 bg-amber-100/50 hover:bg-amber-100/70 transition-colors duration-300">
            ...
            <p className="mb-8 text-lg text-amber-800">ISO certification is not limited to hospitals. Any healthcare-related organization can benefit, including:</p>
            <div className="flex flex-wrap gap-4">
              {['Hospitals & Clinics', 'Nursing Homes', 'Diagnostic Laboratories', 'Pharmaceutical Companies', 'Medical Device Manufacturers', 'Healthcare IT Providers', 'Dental Practices', 'Rehabilitation Centers'].map((item, index) => (
                <span 
                  key={index}
                  className="bg-amber-100 text-amber-900 px-5 py-2 rounded-full font-medium text-lg hover:bg-amber-200 hover:shadow-sm transition-all duration-300"
                >
                  {item}
                </span>
              ))}
            ...
            <ol className="relative border-l-2 border-amber-200 ml-6 space-y-10">
              {[
                ...
                <li key={index} className="relative pl-8 group">
                  ...
                  <p className="text-lg text-amber-800">{item.description}</p>
                </li>
              ))}
            </ol>
          </section>
          ...
          {/* Video Guide Section */}
          <section className="px-8 py-12 sm:px-12 bg-amber-100/50 text-center hover:bg-amber-100/70 transition-colors duration-300">
            <SectionHeader icon={Video} title="Watch a Practical Guide" />
            <p className="mb-8 text-lg text-amber-800 max-w-3xl mx-auto">
              For a step-by-step overview of implementing ISO standards in healthcare, watch our expert guide.
            </p>
            <div className="aspect-w-16 aspect-h-9 bg-amber-100 rounded-xl overflow-hidden flex items-center justify-center relative group border border-amber-200 hover:shadow-md hover:border-amber-300 transition-all duration-300">
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="w-20 h-20 bg-amber-200 rounded-full flex items-center justify-center group-hover:bg-amber-300 group-hover:scale-105 transition-all duration-300">
                  <Video className="w-10 h-10 text-amber-700" />
                </div>
              </div>
