...
// Data constants
const CATEGORIES = [
    { id: 'construction', name: "CONSTRUCTION", icon: <FaBuilding className="text-yellow-400" size={20} />, description: "Construction companies, contractors, and building firms.", standards: [{ id: 'con-9001', code: 'ISO 9001:2015', description: 'Quality management system for construction' }, { id: 'con-14001', code: 'ISO 14001:2015', description: 'Environmental management for construction' }, { id: 'con-45001', code: 'ISO 45001:2018', description: 'Occupational health and safety for sites' }] },
    { id: 'factory', name: "FACTORY / MANUFACTURER", icon: <FaIndustry className="text-yellow-400" size={20} />, description: "Manufacturing plants and production facilities.", standards: [{ id: 'fac-9001', code: 'ISO 9001:2015', description: 'Manufacturing quality standards' }, { id: 'fac-14001', code: 'ISO 14001:2015', description: 'Sustainable manufacturing practices' }, { id: 'fac-45001', code: 'ISO 45001:2018', description: 'Factory worker safety standards' }] },
    { id: 'ingo', name: "INGO/NGO", icon: <FaHandsHelping className="text-yellow-400" size={20} />, description: "International and local non-governmental organizations.", standards: [{ id: 'ing-9001', code: 'ISO 9001:2015', description: 'Quality in humanitarian operations' }, { id: 'ing-26000', code: 'ISO 26000:2010', description: 'Guidance on social responsibility' }, { id: 'ing-sa8000', code: 'SA 8000:2014', description: 'Fair treatment of workers standards' }] },
    { id: 'healthcare', name: "HOSPITAL / CLINIC / LAB", icon: <FaHospital className="text-yellow-400" size={20} />, description: "Healthcare providers, medical laboratories, and clinics.", standards: [{ id: 'med-13485', code: 'ISO 13485:2016', description: 'Quality management for medical devices' }, { id: 'med-15189', code: 'ISO 15189:2012', description: 'Medical laboratory quality standards' }] },
    { id: 'hospitality', name: "HOTEL / FOOD INDUSTRY", icon: <FaUtensils className="text-yellow-400" size={20} />, description: "Hotels, restaurants, and food service businesses.", standards: [{ id: 'hot-22000', code: 'ISO 22000:2018', description: 'Food safety management system' }, { id: 'hot-haccp', code: 'HACCP', description: 'Critical control points for food safety' }] },
    { id: 'finance', name: "BANK / FINANCE / INSURANCE", icon: <FaPiggyBank className="text-yellow-400" size={20} />, description: "Financial institutions requiring data security.", standards: [{ id: 'fin-9001', code: 'ISO 9001:2015', description: 'Financial service quality standards' }, { id: 'fin-27001', code: 'ISO 27001:2022', description: 'Data protection and security management' }] },
    { id: 'education', name: "SCHOOL/COLLEGE EDU", icon: <FaUniversity className="text-yellow-400" size={20} />, description: "Educational institutions for academic quality.", standards: [{ id: 'edu-21001', code: 'ISO 21001:2018', description: 'Quality management in education' }] },
    { id: 'service', name: "SERVICE INDUSTRIES", icon: <FaConciergeBell className="text-yellow-400" size={20} />, description: "Service providers requiring quality delivery.", standards: [{ id: 'ser-9001', code: 'ISO 9001:2015', description: 'Service quality standards' }, { id: 'ser-45001', code: 'ISO 45001:2018', description: 'Service worker safety management' }] },
    { id: 'transport', name: "TRANSPORT / LOGISTIC", icon: <FaTruck className="text-yellow-400" size={20} />, description: "Transportation and logistics companies.", standards: [{ id: 'log-9001', code: 'ISO 9001:2015', description: 'Logistics quality standards' }, { id: 'log-28000', code: 'ISO 28000:2007', description: 'Security in logistics operations' }] },
    { id: 'warehousing', name: "EXIM/WAREHOUSING", icon: <FaWarehouse className="text-yellow-400" size={20} />, description: "Warehousing and import/export businesses.", standards: [{ id: 'war-9001', code: 'ISO 9001:2015', description: 'Warehousing quality standards' }, { id: 'war-45001', code: 'ISO 45001:2018', description: 'Warehouse worker safety procedures' }] },
    { id: 'technology', name: "INFO TECHNOLOGY", icon: <FaServer className="text-yellow-400" size={20} />, description: "IT companies and service providers.", standards: [{ id: 'it-9001', code: 'ISO 9001:2015', description: 'IT service quality standards' }, { id: 'it-27000', code: 'ISO 27000:2018', description: 'IT security best practices' }] },
];
...
const FORM_FIELDS = [
    ...
    { name: 'address', label: 'Address', icon: <FiHome />, placeholder: 'Your Full Address....' },
...
const FormInput = ({ name, label, icon, error, ...props }) => (
    <div className="relative">
        <label className="block text-amber-900 font-bold mb-2 pl-1">{label}</label>
        <div className="relative">
        ...
        return (
            <div className="bg-amber-50 p-6 rounded-xl border border-amber-200 h-full flex flex-col items-center justify-center text-center hover:shadow-md transition-shadow">
                <div className="bg-yellow-100 rounded-full p-3 mb-4">
                    <FiBriefcase className="text-yellow-400" size={40} />
                </div>
    ...
    const handleAddCustomCategory = () => {
        ...
        if (name && !allCategories.some(c => c.name.toLowerCase() === name.toLowerCase())) {
            const newCategory = {
                id: `custom-${Date.now()}`,
                name,
                icon: <FiBriefcase className="text-yellow-400" size={20} />,
                            ...
                            selectedItems.map((key, index) => {
                                ...
                                return (
                                    <span key={item.id || index} className="bg-amber-50 text-amber-900 px-2 py-1 rounded-md text-sm flex items-center hover:bg-amber-100" onClick={(e) => { e.stopPropagation(); onItemClick(item); }}>
                                        ...
                                        <button type="button" onClick={(e) => { e.stopPropagation(); onToggle(displayKey === 'name' ? item.name : item); }} className="ml-2 text-amber-700 hover:text-amber-900"><FiX size={14} /></button>
                                    </span>
                                );
                            })
                        ) : <span className="text-amber-700 pl-2">Select one or more...</span>}
                    </div>
                    {isOpen ? <FiChevronUp className="text-amber-700" /> : <FiChevronDown className="text-amber-700" />}
                </div>
                {error && <p className="text-red-500 text-xs mt-1 flex items-center"><FiAlertCircle /> {error}</p>}
                {isOpen && (
                        ...
                        {items.map(item => (
                            ...
                            >
                                <div className="flex items-center justify-between">
                                    ...
                                    {selectedItems.some(sel => (typeof sel === 'object' ? sel.id === item.id : sel === item[displayKey])) && <FiCheck className="text-amber-700" />}
                                </div>
                            </div>
                        ))}
                    </div>
