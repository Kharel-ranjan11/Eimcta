...
export default function BusinessQuoteForm() {
  ...
  const fetchLocation = async (lat, lon) => {
    try {
      ...
      setFormData(prev => ({
        ...prev,
        address: `${data.city || ''}, ${admin.find(a => a.adminLevel === 6)?.name || ''}, ${admin.find(a => a.adminLevel === 4)?.name || ''}`.trim().replace(/^,\s*|,\s*$/g, ''),
    ...
    <>
      ...
      <main className="min-h-screen p-6  font-['Arial_Narrow']">
        <div className="max-w-6xl mx-auto p-8 bg-white rounded-xl shadow-lg border border-amber-200 hover:shadow-xl transition-shadow duration-300">
          ...
          <p className="text-amber-800 text-center mb-8">Please fill out the form below and we'll get back to you shortly</p>

          <div className="bg-amber-100/50 p-4 rounded-lg mb-6 border border-amber-200">
            <p className="text-amber-800 text-sm">Note: All fields marked with * are required</p>
          </div>
          ...
          <form ref={form} onSubmit={handleSubmit} className="grid grid-cols-1 gap-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="group">
                ...
                <div className="relative">
                  <div className="absolute left-3 top-1/2 -translate-y-1/2 bg-amber-100 p-1.5 rounded-full group-hover:bg-amber-200 transition-colors">
                    <IconUser className="h-4 w-4 text-yellow-400" />
                  </div>
                  <input type="text" name="name" value={formData.name} onChange={handleChange}
                    className={`pl-12 w-full border ${errors.name ? 'border-red-400' : 'border-gray-300'} rounded-lg px-4 py-3 focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500 outline-none transition hover:border-amber-300`}
              ...
              <div className="group">
                ...
                <div className="relative">
                  <div className="absolute left-3 top-1/2 -translate-y-1/2 bg-amber-100 p-1.5 rounded-full group-hover:bg-amber-200 transition-colors">
                    <IconBuilding className="h-4 w-4 text-yellow-400" />
                  </div>
                  <input type="text" name="organization" value={formData.organization} onChange={handleChange}
                    className={`pl-12 w-full border ${errors.organization ? 'border-red-400' : 'border-gray-300'} rounded-lg px-4 py-3 focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500 outline-none transition hover:border-amber-300`}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="group">
                ...
                <div className="relative">
                  <div className="absolute left-3 top-1/2 -translate-y-1/2 bg-amber-100 p-1.5 rounded-full group-hover:bg-amber-200 transition-colors">
                    <IconEnvelope className="h-4 w-4 text-yellow-400" />
                  </div>
                  <input type="email" name="email" value={formData.email} onChange={handleChange}
                    className={`pl-12 w-full border ${errors.email ? 'border-red-400' : 'border-gray-300'} rounded-lg px-4 py-3 focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500 outline-none transition hover:border-amber-300`}
              ...
              <div className="group">
                ...
                <div className="relative">
                  <div className="absolute left-3 top-1/2 -translate-y-1/2 bg-amber-100 p-1.5 rounded-full group-hover:bg-amber-200 transition-colors">
                    <IconPhone className="h-4 w-4 text-yellow-400" />
                  </div>
                  <input type="tel" name="phone" value={formData.phone} onChange={handleChange}
                    className={`pl-12 w-full border ${errors.phone ? 'border-red-400' : 'border-gray-300'} rounded-lg px-4 py-3 focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500 outline-none transition hover:border-amber-300`}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="group">
                ...
                <div className="relative">
                  <div className="absolute left-3 top-1/2 -translate-y-1/2 bg-amber-100 p-1.5 rounded-full group-hover:bg-amber-200 transition-colors">
                    <IconMapMarkerAlt className="h-4 w-4 text-yellow-400" />
                  </div>
                  <input type="text" name="address" value={formData.address} onChange={handleChange}
                    className={`pl-12 w-full border ${errors.address ? 'border-red-400' : 'border-gray-300'} rounded-lg px-4 py-3 focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500 outline-none transition hover:border-amber-300`}
              ...
              <div className="group">
                ...
                <div className="relative">
                  <div className="absolute left-3 top-1/2 -translate-y-1/2 bg-amber-100 p-1.5 rounded-full group-hover:bg-amber-200 transition-colors">
                    <IconGlobe className="h-4 w-4 text-yellow-400" />
                  </div>
                  <input type="text" name="country" value={formData.country} onChange={handleChange}
                    className={`pl-12 w-full border ${errors.country ? 'border-red-400' : 'border-gray-300'} rounded-lg px-4 py-3 focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500 outline-none transition hover:border-amber-300`}
            ...
            <div className="services-dropdown-container group">
              ...
              <div className={`flex flex-wrap gap-2 p-2 border rounded-lg min-h-[50px] transition ${errors.selectedServices ? 'border-red-400' : 'border-gray-300 hover:border-amber-300'}`}>
                {formData.selectedServices.length > 0 ? formData.selectedServices.map(service => (
                  <div key={service} className="flex items-center bg-amber-100 text-amber-800 text-xs font-medium px-2.5 py-1 rounded-full hover:bg-amber-200 transition-colors">
                    <span>{service}</span>
              ...
              <div className="relative mt-2">
                <button type="button"
                  className="w-full border border-gray-300 rounded-lg p-3 cursor-pointer flex justify-between items-center text-left hover:bg-amber-50 transition-colors group/dropdown"
                  onClick={() => setShowDropdown(!showDropdown)}>
                  <span className='text-gray-700 group-hover/dropdown:text-amber-800'>
                    {showDropdown ? 'Close Services List' : 'Add/View Services'}
                  </span>
                  {showDropdown ?
                    <IconTimes className="h-4 w-4 text-amber-600" /> :
                    <IconChevronDown className="h-4 w-4 text-amber-600 group-hover/dropdown:translate-y-0.5 transition-transform" />}
                </button>
                {showDropdown && (
                  <div className="absolute z-10 w-full mt-1 max-h-60 overflow-y-auto border border-amber-200 bg-white rounded-lg shadow-lg p-2">
                    {servicesInterested.map((service, idx) => (
                      <label key={idx} className="flex items-center p-2 rounded-md hover:bg-amber-50 cursor-pointer transition-colors">
                    ...
                    <div className="p-2 mt-2 border-t border-amber-200">
                      {showCustomInput ? (
                        ...
                        <button type="button" onClick={() => setShowCustomInput(true)} className="text-amber-600 hover:underline text-sm hover:text-amber-700 transition-colors">Add other service...</button>
                      )}
                    </div>
                  </div>
                )}
            ...
            <div className="group">
              ...
              <div className="relative">
                <div className="absolute left-3 top-4 bg-amber-100 p-1.5 rounded-full group-hover:bg-amber-200 transition-colors">
                  <IconCommentAlt className="h-4 w-4 text-yellow-400" />
                </div>
            ...
            {submitStatus.message && (
              <div className={`mt-4 p-4 rounded-lg text-center transition-all ${submitStatus.success ? 'bg-green-50 border border-green-200 text-green-800' : 'bg-red-50 border border-red-200 text-red-800'}`}>
                {submitStatus.message}
              </div>
            )}
          </form>
