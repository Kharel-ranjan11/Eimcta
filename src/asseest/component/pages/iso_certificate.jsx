...
const ISO_certification = () => {
  ...
  const centeredUnderlineVariants = {
    hidden: { width: 0 },
    show: { width: '25%', transition: { duration: 0.8, delay: 0.3 } }
  };
  ...
  return (
    <div className="bg-gray-50 min-h-screen font-sans">
      <div className="py-12 px-4 sm:px-6 lg:px-8">
        ...
        >
          {/* Header Section */}
          <header className="bg-amber-600 p-8 text-center rounded-t-2xl">
            <motion.h1
              className="text-4xl sm:text-5xl font-extrabold text-white mb-2 leading-tight"
              variants={slideInRightVariants}
            >
              How to Certify Your Organization with Relevant ISO Standards
            </motion.h1>
            <motion.p
              className="text-amber-100 text-lg sm:text-xl font-light"
          ...
          <section className="p-8 border-b border-gray-100">
            <motion.p
              className="text-gray-700 text-lg mb-4 text-justify"
          ...
          >
            <motion.h2 className="text-3xl font-bold text-amber-600 mb-4 relative inline-block">
          ...
          <motion.section
            ...
            viewport={{ once: true, amount: 0.2 }}
          >
            <motion.h2 className="text-3xl font-bold text-amber-600 mb-6 relative inline-block">
          ...
          >
            <motion.h2 className="text-3xl font-bold text-amber-600 mb-8 relative inline-block">
            ...
            <div className="space-y-8">
              {[
                ...
                return (
                  <motion.div
                    key={index}
                    className={`bg-amber-50 p-6 rounded-xl shadow-sm hover:translate-y-[-8px] transition-transform duration-300 hover:shadow-2xl`}
                    variants={stepVariants(index)}
                  >
                    <h3 className="text-xl font-bold text-amber-700 mb-2 flex items-center">
                      <span className="bg-amber-600 text-white rounded-full h-8 w-8 flex items-center justify-center font-bold text-sm mr-4">{index + 1}</span>
                      <IconComponent className="h-6 w-6 mr-2 text-amber-700" />
                      {step.title}
                    </h3>
                    <p className="text-gray-700 text-justify">{step.content}</p>
                    {step.list && (
                      <ul className="list-disc pl-14 mt-2 text-gray-700">
                        {step.list.map((item, i) => (
                          <li key={i}><span className="font-semibold text-amber-700">{item.split(':')[0]}:</span> {item.split(':')[1]}</li>
                        ))}
                      </ul>
                    )}
                  </motion.div>
